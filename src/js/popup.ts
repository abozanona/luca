...
var actionsOrder = ['NOTHING', 'WAITING_CREATE_ROOM', 'WAITING_SELECT_VIDEO', 'ROOM_SETUP_COMPLETED'];
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    switch (request.code) {
        case 'A_GET_PAGE_STATUS':
            ...
            currentAction = [status, currentAction].sort(
                (a, b) => actionsOrder.indexOf(b) - actionsOrder.indexOf(a)
            )[0];
            renderPopupScreen(currentAction);
            break;
    }
